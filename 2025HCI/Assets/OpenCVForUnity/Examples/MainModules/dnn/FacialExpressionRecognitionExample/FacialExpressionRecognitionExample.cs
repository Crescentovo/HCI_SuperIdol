...
namespace OpenCVForUnityExample
{
    ...
    public class FacialExpressionRecognitionExample : MonoBehaviour
    {
        ...
        public Action<Emotion, float> OnEmotionUpdated;
                    ...
                    else
                    {
                        ...
                        using (Mat faces = _faceDetector.Detect(_bgrMat))
                        {
                            ...
                            using (Mat facialExpressions = _facialExpressionRecognizer.Recognize(_bgrMat, faces))
                            {
                                ...
                                // === 添加接口 把“插件已经算好的结果”抛出去 ===
                                if (OnEmotionUpdated != null && facialExpressions.rows() > 0)
                                {
                                    var best = _facialExpressionRecognizer.GetBestMatchData(facialExpressions, 0);
                                    OnEmotionUpdated.Invoke(
                                        (Emotion)best.ClassId,
                                        best.Confidence
                                    );
                                }
                            }
