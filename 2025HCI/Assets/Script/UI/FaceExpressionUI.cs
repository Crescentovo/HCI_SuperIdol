...
public class FaceExpressionUI : MonoBehaviour
{
    ...
    {
        // 注册按钮点击事件
        neutralBtn.onClick.AddListener(() => SetExpression(GameProgress.FacialExpression.Neutral));
        happyBtn.onClick.AddListener(() => SetExpression(GameProgress.FacialExpression.Happy));
        surprisedBtn.onClick.AddListener(() => SetExpression(GameProgress.FacialExpression.Surprised));
        angryBtn.onClick.AddListener(() => SetExpression(GameProgress.FacialExpression.Angry));
    ...
    private void Update()
    {
        ...
        //UpdatePointer();
        if (FindObjectOfType<GameProgress>().IsFacialDetectionStarted)
        {
    ...
    /// 修改当前表情并更新指示箭头
    /// </summary>
    private void SetExpression(GameProgress.FacialExpression expression)
    {
